...
{pageset:}
	...
	<div class="settings alert alert-info" style="display:none">
		Сортировать <a style="font-weight:{data.md.sort??:bold}" data-anchor='.pagination' href='?{infrajs.ids.catalog.crumb}{:cat.mark.add}sort'>по умолчанию</a>,
			<a style="font-weight:{data.md.sort=:name?:bold}" data-anchor='.pagination' href='?{infrajs.ids.catalog.crumb}{:cat.mark.add}sort=name'>по названию</a>, 
			<a style="font-weight:{data.md.sort=:cost?:bold}"data-anchor='.pagination' href='?{infrajs.ids.catalog.crumb}{:cat.mark.add}sort=cost'>по цене</a>, 
			<a style="font-weight:{data.md.sort=:change?:bold}" data-anchor='.pagination' href='?{infrajs.ids.catalog.crumb}{:cat.mark.add}sort=change'>по дате</a><br>
		Показывать по
		<select onchange="infra.Crumb.go('?{infrajs.ids.catalog.crumb}{:cat.mark.add}count='+$(this).val()); ascroll.go('.pagination');">
